#
# this Power Shell script will delete the deployment
...
if ($args.length -lt 2)
{
   Write-Host "Usage: AzureDeleteDeployment.ps1 <PublishProfile> <ConnectionsXml> [DeploymentSlot] "
...
while (1)
{
    ...
    if ($error)
    {
        Write-host "Unable to delete deployment"
        $retries -= 1
        if ($retries -eq 0)
        {
            exit 888
