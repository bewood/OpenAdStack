{
	"Campaign" : {
		"ExternalEntityId" : "8b9a3f92cb1d4f5dbd5e24ac951d0538",
		"EntityCategory" : "Campaign",
		"CreateDate" : "2012-05-14T15:20:46.0000000Z",
		"LastModifiedDate" : "2012-06-15T20:42:15.0768970Z",
		"LastModifiedUser" : "1AD0DD27B0DE4605B8DA6F5C6C26A9E8",
		"LocalVersion" : "553",
		"ExternalName" : "Forbes Campaign for Simulating",
		"ExternalType" : "DynamicAllocationCampaign",
		"Properties" : {
			"Status" : "Approved",
			"Budget" : "10000",
			"StartDate" : "2013-02-18T00:00:27.0000000Z",
			"EndDate" : "2013-04-18T00:00:41.0000000Z",
			"DA_InitialAllocationDone" : "false",
			"APNX_LineItemId" : "148991",
			"RemainingBudget" : "10000",
			"APNX_LifetimeMediaBudgetCap" : "10000"
		},
		"ExtendedProperties" : {
			"MeasureSets" : {
				"IdealValuation" : "20.0",
				"MaxValuation" : "57.4",
				"Measures" : [{
						"measureId" : "107000000000018024",
						"group" : "group1",
						"pinned" : false
					}, {
						 
						"measureId" : "107000000000025034",
						"group" : "group1",
						"pinned" : false
					}, {
						 
						"measureId" : "107000000000035044",
						"group" : "group1",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100063",
						"group" : "group1",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100064",
						"group" : "group1",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100068",
						"group" : "group2",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100069",
						"group" : "group2",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100073",
						"group" : "group3",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100085",
						"group" : "group3",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100087",
						"group" : "group3",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100091",
						"group" : "group3",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100092",
						"group" : "group3",
						"pinned" : false
					}, {
						 
						"measureId" : "110000000000100098",
						"group" : "group3",
						"pinned" : false
					}
				]
			},
			"valuationSet" : [{
					"measureId" : "107000000000018024",
					"valuation" : 50
				}, {
					"measureId" : "107000000000025034",
					"valuation" : 50
				}, {
					"measureId" : "107000000000035044",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100063",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100064",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100068",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100069",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100073",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100085",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100087",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100091",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100092",
					"valuation" : 50
				}, {
					"measureId" : "110000000000100098",
					"valuation" : 50
				}
			],
			"NodeValuationSet" : []
		},
		"SystemProperties" : {
			"SimulatorConfig" : {
				"RandPercentage" : "0.50",
				"BaseTierDeliveryProbability" : ".85",
				"DeliveryProbilityDecayRate" : ".75",
				"RandomSeed" : "211",
				"AverageEcpm" : "1.75",
				"RateBaseTier" : "4",
				"RateTopTier" : "6",
				"BaseTierRate" : ".85",
				"AllocationTopTierRate" : ".01",
				"MeasureRateDeviationPercentage" : "1.05",
				"DeliverySimulationType" : "2"
			},
			"AllocationParameters" : {
				"DynamicAllocation.DefaultEstimatedCostPerMille" : "1.5",
				"DynamicAllocation.Margin" : "1.17647059",
				"DynamicAllocation.PerMilleFees" : "0",
				"DynamicAllocation.BudgetBuffer" : "1.1",
				"DynamicAllocation.InitialAllocationTotalPeriodDuration" : "1.00:00:00",
				"DynamicAllocation.InitialAllocationSinglePeriodDuration" : "6:00:00",
				"DynamicAllocation.AllocationTopTier" : "6",
				"DynamicAllocation.AllocationNumberOfTiersToAllocateTo" : "3",
				"DynamicAllocation.AllocationNumberOfNodes" : "150", 
				"DynamicAllocation.MaxNodesToExport" : "175",
				"DynamicAllocation.UnderSpendExperimentNodeCount" : "10",
				"DynamicAllocation.UnderSpendExperimentTier" : "3",
				"DynamicAllocation.MinBudget" : ".60",
				"DynamicAllocation.ExportBudgetBoost" : "1",
				"DynamicAllocation.LargestBudgetPercentAllowed" : ".02",
				"DynamicAllocation.NeutralBudgetCappingTier" : "4",
				"DynamicAllocation.LineagePenalty" : ".1",
				"DynamicAllocation.LineagePenaltyNeutral" : "1",
				"DynamicAllocation.MinimumImpressionCap" : "100",
				"DynamicAllocation.InitialMaxNumberOfNodes" : "75" 
			}
		}
	}
}